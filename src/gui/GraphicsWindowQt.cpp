...
void GraphicsWindowQt::GLWidget::keyPressEvent(QKeyEvent* event)
{
    ...
    _gwin->getEventQueue()->keyPress(remapQt2OSG(event->key()));
}
...
void GraphicsWindowQt::GLWidget::keyReleaseEvent(QKeyEvent* event)
{
    ...
    {
        setKeyboardModifiers(event);
        _gwin->getEventQueue()->keyRelease(remapQt2OSG(event->key()));
    }
}
...
void GraphicsWindowQt::GLWidget::wheelEvent(QWheelEvent* event)
{
    ...
    {
        _gwin->getEventQueue()->mouseScroll(
...
void GraphicsWindowQt::GLWidget::enqueueDeferredEvent(QEvent::Type eventType, QEvent::Type removeEventType)
{
    QMutexLocker lock(&_deferredEventQueueMutex);
