...
units::mass::kilogram_t TailVer::GetEstimatedMass(const AircraftData& data)
{
    ...
    mass::pound_t m2 = 0.0_lb;
    {
        ...
        if ( data.type == AircraftData::FighterAttack )
        {
            ...
            m2 = 0.452_lb * k_rht * pow(1.0 + ht_hv, 0.5) * pow(w_dg() * n_z, 0.488)
                    * pow(s_vt(), 0.718) * pow(data.general.mach_max, 0.341) * pow(l_t(), -1.0)
                    * pow(1.0 + s_r / s_vt, 0.348) * pow(data.ver_tail.ar, 0.223)
                    * pow(1.0 + data.ver_tail.tr, 0.25) * pow(cos(sweep()), -0.323);
        }

        // Rayner: Aircraft Design, p.574, eq.15.27
        if ( data.type == AircraftData::CargoTransport )
        {
            length::foot_t k_z = l_t;

            m2 = 0.0026_lb * pow(1.0 + ht_hv, 0.225) * pow(w_dg(), 0.556)
                    * pow(n_z, 0.536) * pow(l_t(), -0.5) * pow(s_vt(), 0.5)
                    * pow(k_z(), 0.875) * pow(cos(sweep()), -1.0)
                    * pow(data.ver_tail.ar, 0.35) * pow(data.ver_tail.tc, -0.5);
        }
        ...
        if ( data.type == AircraftData::GeneralAviation )
        {
            ...
            m2 = 0.073_lb * (1.0 + 0.2 * ht_hv) * pow(n_z * w_dg(), 0.376)
                    * pow(q(), 0.122) * pow(s_vt(), 0.873)
        ...
        if ( data.type == AircraftData::Helicopter )
        {
            ...
            m2 = chi_vt * 1.046_lb * f_tr * pow(s_vt(), 0.9441) * pow(data.ver_tail.ar, 0.5332);
            m1 = m2;
        }
    }
