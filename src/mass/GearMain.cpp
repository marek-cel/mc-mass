...
units::mass::kilogram_t GearMain::GetEstimatedMass(const AircraftData& data)
{
    ...
    mass::pound_t m1 = 0.0_lb;
    {
        ...
        if ( data.type == AircraftData::FighterAttack )
        {
            ...
            m1 = 0.85_lb * (coeff * w_dg() - reduction);
        }

        // Rayner: Aircraft Design, p.568, table 15.2
        if ( data.type == AircraftData::CargoTransport )
        {
            m1 = 0.85_lb * (0.043 * w_dg() - reduction);
        }

        // Rayner: Aircraft Design, p.568, table 15.2
        if ( data.type == AircraftData::GeneralAviation )
        {
            m1 = 0.85_lb * (0.057 * w_dg() - reduction);
        }
    ...
    mass::pound_t m2 = 0.0_lb;
    {
        ...
        if ( data.type == AircraftData::FighterAttack )
        {
            ...
            m2 = 1.0_lb * k_cb * k_tpg * pow(w_l() * n_l, 0.25) * pow(l_m(), 0.973);
        }
        ...
        if ( data.type == AircraftData::CargoTransport )
        {
            ...
            m2 = 0.0106_lb * k_mp * pow(w_l(), 0.888) * pow(n_l, 0.25) * pow(l_m(), 0.4)
        ...
        if ( data.type == AircraftData::GeneralAviation )
        {
            m2 = 0.095_lb * pow(n_l * w_l(), 0.768) * pow(l_m() / 12.0, 0.409);
        }

        // NASA TP-2015-218751, p.233
        if ( data.type == AircraftData::Helicopter )
        {
            m2 = 0.4013_lb * pow(w_dg(), 0.6662) * pow(n_l, 0.536);
        }
    }
